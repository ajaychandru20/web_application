...
app.post("/submit", (req, res) => {
  ...
  const adjectiveIndex = Math.floor(Math.random() * adj.length);
  const nounIndex = Math.floor(Math.random() * noun.length)
  const adjective = adj[adjectiveIndex];
  const nounWord = noun[nounIndex];
  const combainWords = `<h1> ${adjective} ${nounWord}</h1>`
  res.render('index.ejs', { combainWords });
